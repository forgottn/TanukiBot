...
class URLPlaylistEntry(BasePlaylistEntry):
    def __init__(self, playlist, url, title, duration=0, expected_filename=None, weight=1, **meta):
        super().__init__()
    ...
    def upvote(self, author):
        if author.id in self.upvoters:
            return False

        if author.id in self.downvoters:
            self.weight += 1
            self.downvoters.remove(author.id)

        self.weight += 1
        self.upvoters.add(author.id)
        return True
    ...
    def downvote(self, author):
        ...
        if author.id in self.upvoters:
            self.weight -= 1
            self.upvoters.remove(author.id)
            if self.weight == 1:
                return True

        self.weight -= 1
        self.downvoters.add(author.id)
        return True
