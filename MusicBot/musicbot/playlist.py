...
from .utils import get_header, append_playlist
...
class Playlist(EventEmitter):
    ...
    def __init__(self, bot):
        ...
        self.entries = []
        self.autoplaylist = set()
        self.loop_shuffle = False
    ...
    async def playnext(self, next_song_number):
        self.entries.insert(0, self.entries.pop(next_song_number - 1))

    def remove(self, song_number):
        self.entries.pop(song_number - 1)

    async def add_entry(self, song_url, weight=1, **meta):
        """
        ...
        if song_url not in self.bot.autoplaylistset:
            append_playlist(self.bot.config.auto_playlist_file, song_url)
            self.bot.autoplaylist.append((song_url, "1"))
            self.bot.autoplaylistset.add(song_url)
            self.bot.autoplaylistindexes[(song_url, "1")] = self.bot.last_line
            self.bot.last_line += 1
        else:
            weight = int([weight for song, weight in self.bot.autoplaylist if song == song_url][0])
    ...
    def _add_entry(self, entry):
        if self.loop_shuffle:
            self.entries.insert(randint(0, len(self.entries)), entry)
        else:
