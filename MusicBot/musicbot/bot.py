...
from musicbot.utils import load_file, write_file, sane_round_int, append_playlist, load_playlist, write_playlist
...
class MusicBot(discord.Client):
    def __init__(self, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        ...
        self.autoplaylist, self.autoplaylistindexes, self.last_line = load_playlist(self.config.auto_playlist_file)
        write_playlist(self.config.auto_playlist_file, self.autoplaylist)
        self.autoplaylistset = set([playlist for playlist, vote in self.autoplaylist])
        ...
        for channel in channels:
            ...
            if channel and channel.type == discord.ChannelType.voice:
                ...
                try:
                    ...
                    self.previous_weight[channel.server.id] = 0
    ...
    async def on_player_play(self, player, entry):
        ...
        self.previous_weight[player.voice_client.server.id] = entry.weight
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                index = (entry.url, str(self.previous_weight[player.voice_client.server.id]))
                if entry.upvote(author):
                    print("[Bot] is upvoting for you")
                    self.autoplaylist[self.autoplaylistindexes[index]] = (entry.url, str(entry.weight))
                self.last_played_song = entry
                newmsg = '%s - your song **%s** is now playing, b-baka in %s!' % (
    ...
    async def on_player_finished_playing(self, player, **_):
        if _ and _['entry'] and self.previous_weight[player.voice_client.server.id] != _['entry'].weight:
            index = (_['entry'].url, str(self.previous_weight[player.voice_client.server.id]))
            new_index = (_['entry'].url, str(_['entry'].weight))
            self.autoplaylistindexes[new_index] = self.autoplaylistindexes.pop(index)

        write_playlist(self.config.auto_playlist_file, self.autoplaylist)

        if _ and _['entry'] and self.loop_queue and not player.loop_song:
            song_url = _['entry'].url
            return await player.playlist.add_entry(song_url, channel=None, author=None)

        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            while self.autoplaylist:
                song_url, weight = self._weighted_choice(self.autoplaylist)
                info = await self.downloader.safe_extract_info(player.playlist.loop, song_url, download=False, process=False)

                if not info:
                    song_to_remove = [(url, weight) for url, weight in self.autoplaylist if song_url == url][0]
                    self.autoplaylist.remove(song_to_remove)
                ...
                try:
                    await player.playlist.add_entry(song_url, weight=weight, channel=None, author=None)
    ...
    async def cmd_commands(self, command=None):
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "It's not like I wanted to play **%s** for you. B-baka. Position in queue: %s"
            btext = entry.title
    ...
    async def cmd_playnext(self, player, next_song):
        """
        Usage:
            {command_prefix}playnext [next_song]

        Moves the song number in the playlist to the front.
        """
        try:
            next_song_number = int(next_song)

        except ValueError:
            raise exceptions.CommandError('{} is not a valid number'.format(next_song), expire_in=20)
        
        if 0 < next_song_number <= len(player.playlist):
           await player.playlist.playnext(next_song_number)
        else:
            raise exceptions.CommandError(
                    'Unreasonable playlist number provided: {}. Provide a value between 1 and {}.'.format(
                        next_song_number, len(player.playlist)), expire_in=20)

        return Response("Moved song to front!", delete_after=15)

    async def cmd_remove(self, player, song_number):
        """
        Usage:
            {command_prefix}remove [song_number]

        Removes the song at song_number in the playlist
        """
        try:
            song_number_to_remove = int(song_number)

        except ValueError:
            raise exceptions.CommandError('{} is not a valid number'.format(song_number), expire_in=20)
        
        if 0 < song_number_to_remove <= len(player.playlist):
            player.playlist.remove(song_number_to_remove)
        else:
            raise exceptions.CommandError(
                    'Unreasonable playlist number provided: {}. Provide a value between 1 and {}.'.format(
                        song_number_to_remove, len(player.playlist)), expire_in=20)

        return Response("Removed song from queue!", delete_after=15)

    async def cmd_history(self, player, number_of_songs=10):
        """
        Usage:
            {command_prefix}history [number_of_songs]

        Lists the past [number_of_songs] played.
        """
        try:
            number_of_songs = int(number_of_songs)

        except ValueError:
            raise exceptions.CommandError('{} is not a valid number'.format(number_of_songs), expire_in=20)

        lines = []
        unlisted = 0
        andmoretext = '* ... and %d more*' % (len(player.history))

        if 0 < number_of_songs <= 100:
            min_num = number_of_songs if number_of_songs < len(player.history) else len(player.history)
            if min_num == 0:
                lines.append(
                    'There are no songs in history! Play something with {}play.'.format(self.config.command_prefix))
            else:
                lines.append("Past {} song(s) played:\n".format(min_num))
            ...
            raise exceptions.CommandError('Provide a value between 1 and 100.', expire_in=20)

        return Response(message, delete_after=60)

    async def cmd_setvotes(self, player, votes):
        """
        Usage:
            {command_prefix}setvotes [votes]

        Sets the votes to [votes]
        """
        try:
            votes = int(votes)

        except ValueError:
            raise exceptions.CommandError('{} is not a valid number'.format(votes), expire_in=20)
        ...
        player.current_entry.weight = votes
        index = (player.current_entry.url, str(self.previous_weight[player.voice_client.server.id]))
        new_index = (player.current_entry.url, str(votes))
        self.autoplaylist[self.autoplaylistindexes[index]] = (player.current_entry.url, str(player.current_entry.weight))
        self.autoplaylistindexes[new_index] = self.autoplaylistindexes.pop(index)
        write_playlist(self.config.auto_playlist_file, self.autoplaylist)
        self.previous_weight[player.voice_client.server.id] = votes
        return Response(
            "I'm just here because I had nothing else to do! Set the vote! The count for **{}** is now **{}**".format(
                player.current_entry.title, votes
            ),
            delete_after=20
        )

    async def cmd_upvote(self, player, author):
        """
        Usage:
            {command_prefix}upvote

        Upvotes the current song.
        """
        if player.is_stopped:
            raise exceptions.CommandError("Can't upvote! The player is not playing!", expire_in=20)
        ...
        index = (player.current_entry.url, str(self.previous_weight[player.voice_client.server.id]))
        if player.current_entry.upvote(author):
            self.autoplaylist[self.autoplaylistindexes[index]] = (player.current_entry.url, str(player.current_entry.weight))
            return Response(
                "I'm just here because I had nothing else to do! Upvoted! The count for **{}** is now **{}**".format(
                    player.current_entry.title, player.current_entry.weight
                ),
                delete_after=20
            )
        else:
            return Response("B-baka. You've already upvoted this song once during this play", delete_after=10)

    async def cmd_downvote(self, player, author):
        """
        Usage:
            {command_prefix}downvote

        Downvotes the current song.
        """
        if player.is_stopped:
            raise exceptions.CommandError("Can't downvote! The player is not playing!", expire_in=20)
        ...
        if player.current_entry.weight == 1:
            return Response("Cannot downvote anymore", delete_after=10)

        index = (player.current_entry.url, str(self.previous_weight[player.voice_client.server.id]))
        if player.current_entry.downvote(author):
            self.autoplaylist[self.autoplaylistindexes[index]] = (player.current_entry.url, str(player.current_entry.weight))
            return Response(
                "I'm just here because I had nothing else to do! Downvoted! The count for **{}** is now **{}**".format(
                    player.current_entry.title, player.current_entry.weight
                ),
                delete_after=20
            )
        else:
            return Response("You've already downvoted this song once during this play", delete_after=10)

    async def cmd_votes(self, player):
        """
        Usage:
            {command_prefix}votes

        Shows the votes on the current song.
        """
        if player.is_stopped:
            raise exceptions.CommandError("Are you stupid? The player is not playing!", expire_in=20)
        ...
        if not player.current_entry:
            ...
            return Response(
                "HEY! It's a privilege to even be able to talk to me! You should be honored! The count for **{}** is **{}**".format(
                    player.current_entry.title, player.current_entry.weight
                ),
                delete_after=20
            )

    async def cmd_playtop(self, player, channel, author, permissions, leftover_args, number="1"):
        """
        Usage:
            {command_prefix}playtop <number>

        Plays the top upvoted <number> song. It breaks ties by choosing one at random."
        """
        sorted_autoplaylist = sorted(self.autoplaylist, key=lambda tup: int(tup[1]), reverse=True)
        if (re.findall(r'[0-9]+-[0-9]+', str(number))):
            start, end = number.split('-')
            for i in range(int(start), int(end)+1):
                song_url = sorted_autoplaylist[i-1][0]
                await self.cmd_play(player, channel, author, permissions, leftover_args, song_url)
            return Response(
                "Queued up **{}** songs".format(int(end-start)), delete_after=20)

        try:
            top_song = int(number)
        except ValueError:
            raise exceptions.CommandError('{} is not a valid number, b-baka'.format(number), expire_in=20)

        if top_song == 1:
            song_urls = [song_url for song_url, weight in sorted_autoplaylist if weight == sorted_autoplaylist[0][1]]
            song_url_chosen = choice(song_urls)
            return await self.cmd_play(player, channel, author, permissions, leftover_args, song_url_chosen)
        else:
            song_url = sorted_autoplaylist[top_song-1][0]
            return await self.cmd_play(player, channel, author, permissions, leftover_args, song_url)

    async def cmd_ranking(self, player, channel, number_of_songs=10):
        """
        Usage:
            {command_prefix}ranking [number_of_songs]

        Lists the top [number_of_songs] played.
        """
        return await self.cmd_top(player, channel, number_of_songs)

    async def cmd_rankings(self, player, channel, number_of_songs="10"):
        """
        Usage:
            {command_prefix}rankings [number_of_songs]

        Lists the top [number_of_songs] played.
        """
        return await self.cmd_top(player, channel, number_of_songs)

    async def cmd_top(self, player, channel, number_of_songs="10"):
        """
        Usage:
            {command_prefix}top [number_of_songs]

        Lists the top [number_of_songs] played.
        """
        start = 0
        if (re.findall(r'[0-9]+-[0-9]+', str(number_of_songs))):
            start = int(number_of_songs.split('-')[0]) - 1
            end = int(number_of_songs.split('-')[1])
            number_of_songs = end - start
        else:
            try:
                end = int(number_of_songs)
                number_of_songs = int(number_of_songs)
        ...
        unlisted = 0
        sorted_autoplaylist = sorted(self.autoplaylist, key=lambda tup: int(tup[1]), reverse=True)
        andmoretext = '* ... and %d more*' % (len(sorted_autoplaylist))

        if 0 < number_of_songs <= 10:
            process_message = await self.send_message(channel, "Processing...")
            lines.append("Top {} song(s) played:\n".format(number_of_songs))
            for i, item in enumerate(sorted_autoplaylist[start:], start=start):
                if i == end:
                    break

                try:
                    info = await self.downloader.extract_info(player.playlist.loop, item[0], download=False)
                except Exception as e:
                    raise ExtractionError('Could not extract information from {}\n\n{}'.format(song_url, e))

                nextline = '`{}.` **{}** at **{}** votes'.format(i+1, info.get('title', 'Untitled'), item[1]).strip()
            ...
            message = '\n'.join(lines)
            await self.safe_delete_message(process_message, quiet=True)
        elif 0 < number_of_songs <= 100:
            lines.append("Top {} song(s) played:\n".format(number_of_songs))
            for i, item in enumerate(sorted_autoplaylist):
                if i == number_of_songs:
                    break

                nextline = '`{}.` <{}> at **{}** votes'.format(i+1, item[0], item[1]).strip()
            ...
            raise exceptions.CommandError('Are you dumb? Provide a value between 1 and 100.', expire_in=20)

        return Response(message, delete_after=60)

    async def cmd_removesongs(self, player, author, user_mentions):
        """
        Usage:
            {command_prefix}removeme [@user]

        Removes all songs queued by you in the playlist.
        """
        id_to_check = author.id
        numbers_to_remove = []
        if user_mentions:
            id_to_check = user_mentions[0].id
        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('author', False) and item.meta['author'].id == id_to_check:
                numbers_to_remove.insert(0, i)

        if numbers_to_remove:
            for i in numbers_to_remove:
                player.playlist.remove(i)
            if user_mentions:
                return Response("**{}**'s songs have been removed. You better be grateful".format(user_mentions[0].name), reply=True, delete_after=15)
            else:
                return Response("Your songs have been removed. You better be grateful", reply=True, delete_after=15)
        else:
            if user_mentions:
                return Response("Are you stupid? No songs queued by **{}**".format(user_mentions[0].name), reply=True, delete_after=15)
            else:
                return Response("Are you stupid? No songs queued by you", reply=True, delete_after=15)

    async def cmd_url(self, player):
        """
        Usage:
            {command_prefix}url

        Prints the current song's url.
        """
        if player.is_stopped:
            raise exceptions.CommandError("The player is not playing, b-baka!", expire_in=20)

        if player.current_entry:
            return Response("I just happened to be free. The url for: **{}** is <{}>".format(player.current_entry.title, player.current_entry.url), delete_after=20)
        else:
            return Response("Something broke. Please tell me what you did.", delete_after=20)

    async def cmd_timeleft(self, player):
        """
        Usage:
            {command_prefix}timeleft

        Prints the time left for the whole playlist.
        """
        time_until = await player.playlist.estimate_time_until(len(player.playlist)+1, player)
        return Response("Time left on the whole playlist, b-baka: {}".format(time_until), delete_after=15)

    async def cmd_cookie(self, user_mentions):
        """
        Usage:
        ...
        """
        return Response(":cookie:", delete_after=15)

    async def cmd_deletesong(self, player, channel, author, message, permissions, voice_channel):
        """
        Usage:
            {command_prefix}deletesong

        Deletes the current song from the autoplaylist.
        """
        index = (player.current_entry.url, str(self.previous_weight[player.voice_client.server.id]))
        playlist_entry = self.autoplaylist.pop(self.autoplaylistindexes[index])
        self.autoplaylistset.remove(playlist_entry[0])

        new_autoplaylist_indexes = {}
        line = 0
        for entry in self.autoplaylist:
            new_autoplaylist_indexes[entry] = line
            line += 1

        self.last_line = line
        self.autoplaylistindexes = new_autoplaylist_indexes

        await self.cmd_skip(player, channel, author, message, permissions, voice_channel)

        return Response("It's not like I wanted to remove this from the autoplaylist, b-baka", delete_after=10)

    async def cmd_replay(self, player, channel, author, permissions, leftover_args):
        """
        Usage
            {command_prefix}replay

        Replays the last played song by a person.
        """
        if self.last_played_song:
            return await self.cmd_play(player, channel, author, permissions, leftover_args, self.last_played_song.url)
        else:
            raise exceptions.CommandError("b-baka! No one has ever queued up a song before.", expire_in=20)

    async def cmd_current(self, player, channel, server, message):
        """
        Usage:
            {command_prefix}current

        Returns the current song playing.
        """
        return await self.cmd_np(player, channel, server, message)

    async def cmd_autoplaylistcount(self):
        """
        Usage:
            {command_prefix}autoplaylistcount

        Returns the number of songs in the random song playlist.
        """
        return Response("The current random playlist has **{}** songs, b-baka".format(len(self.autoplaylist)))

    async def cmd_loopsong(self, player):
        """
        Usage:
            {command_prefix}loopsong

        Toggles the looping of the song.
        """
        player.loop_song = not player.loop_song;
        if player.loop_song:
            return Response("It's not like I wanted to loop the song for you, b-baka.", delete_after=20)
        else:
            return Response("Hmph. You should be happy I even looped it for you in the first place.", delete_after=20)

    async def cmd_loopqueue(self, player):
        """
        Usage:
            {command_prefix}loopsong
        ...
        if self.loop_queue:
            return Response("It's not like I wanted to loop the queue for you, b-baka.", delete_after=20)
        else:
            player.playlist.loop_shuffle = False
            return Response("Hmph. You should be happy I even looped it for you in the first place.", delete_after=20)

    async def cmd_loopstatus(self, player):
        """
        Usage:
            {command_prefix}loopstatus

        Checks the loop status.
        """
        if player.loop_song and self.loop_queue:
            return Response("Both the song and queue are looping, b-baka", delete_after=20)
        elif player.loop_song:
            return Response("How dumb can you be to not know that the song is looping", delete_after=20)
        elif self.loop_queue:
            return Response("How dumb can you be to not know that the queue is looping", delete_after=20)
        else:
            return Response("How dumb do you have to be to ask me that nothing is looping", delete_after=20)

    async def cmd_loopshuffle(self, player):
        """
        Usage:
            {command_prefix}loopshuffle

        Shuffles a looping queue.
        """
        if not self.loop_queue:
            return Response("Queue is not looped")

        player.playlist.loop_shuffle = not player.playlist.loop_shuffle
        if player.playlist.loop_shuffle:
            return Response("It's not like I toggled on the shuffle for you! I did it because I had freetime, that's all! ┐(￣ヘ￣;)┌", delete_after=20)
        else:
            return Response("It's not like I toggled off the shuffle for you! I did it because I had freetime, that's all! ┐(￣ヘ￣;)┌", delete_after=20)

    async def cmd_kpop(self, player, channel, author, permissions, leftover_args):
        """
        Usage:
            {command_prefix}kpop

        Plays a kpop playlist.
        """
        song_url = "https://www.youtube.com/playlist?list=PLs5m3_UaKS4bdVHu2r55jfuuvf7-c4tBz"
        return await self.cmd_play(player, channel, author, permissions, leftover_args, song_url)

    async def cmd_nanisore(self, author):
        """
        Usage:
            {command_prefix}nanisore

        Plays the nanisore sound.
        """
        voice_client = await self.get_voice_client(author.voice_channel)
        player = voice_client.create_ffmpeg_player('sounds/nani-soreeeee.mp3')
        player.start()
    ...
    async def cmd_queue(self, channel, player):
        ...
        if not lines:
            lines.append(
                'T-Tch! S-Shut up! There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix))
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response("You're such a slob! Cleaned up {} message{}.".format(len(deleted), 's' * bool(deleted)), delete_after=15)
    ...
    async def cmd_dontend(self):
        """
        Usage:
            {command_prefix}dontend
        """
        return Response("<:FeelsBadMan:283411006321852427> 🔫 Jordan's stream ends and so does my life <:FeelsBadMan:283411006321852427> 🔫")
    ...
    async def cmd_restart(self, channel):
        write_playlist(self.config.auto_playlist_file, self.autoplaylist)
        await self.safe_send_message(channel, "Don't misunderstand, it's not like I like you or anything...")
        await self.disconnect_all_voice_clients()
